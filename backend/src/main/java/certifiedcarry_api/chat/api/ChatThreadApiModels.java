...
record ConversationSummaryResponse(
  ...
  static ConversationSummaryResponse fromServiceRow(Map<String, Object> row) {
    Object unreadCountValue = row.get("unreadCount");
    int unreadCount =
        unreadCountValue instanceof Number numberValue ? numberValue.intValue() : 0;
  ...
  static List<ConversationSummaryResponse> fromServiceRows(List<Map<String, Object>> rows) {
    return rows.stream().map(ConversationSummaryResponse::fromServiceRow).toList();
  }
