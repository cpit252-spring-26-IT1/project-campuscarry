...
public class ChatThreadController {
  ...
  public List<ConversationSummaryResponse> getConversationSummaries(
      @RequestParam(name = "all", defaultValue = "false") boolean includeAllConversations,
      Authentication authentication,
      HttpServletRequest request) {
    ActorRequestContext actor = ActorRequestResolver.requireActor(authentication, request);
    boolean includeAll = actor.isAdmin() && includeAllConversations;
    return ConversationSummaryResponse.fromServiceRows(
        chatService.getConversationSummariesForActor(actor.userId(), includeAll));
  }
  ...
  public List<ChatMessageResponse> getConversationMessages(
    ...
    return ChatMessageResponse.fromServiceRows(
        chatService.getConversationMessagesForActor(threadId, actor.userId(), includeAll));
  }
