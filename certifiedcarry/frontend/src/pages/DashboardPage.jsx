...
const DashboardPage = () => {
  ...
  const loadPlayerDashboard = useCallback(async () => {
    ...
    try {
      ...
      const notifications = await getUnreadChatNotifications(user.id, { forceRefresh: true });
  ...
  const loadRecruiterDashboard = useCallback(async () => {
    ...
    try {
      const [dmOpenness, conversations, notifications, candidates] = await Promise.all([
