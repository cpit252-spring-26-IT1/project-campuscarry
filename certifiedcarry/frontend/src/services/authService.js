...
const completePendingSignupRegistration = async () => {
  ...
  await ensurePendingRecruiterQueueRecord(user);

  const profileImage = await getPlayerProfileImage(user);
  const authUser = buildAuthUser(user, profileImage);
...
const login = async ({ email, password }) => {
  ...
  try {
    ...
    return {
      token: firebaseSession.idToken,
      user: buildAuthUser(user, profileImage),
    };
  } catch (error) {
