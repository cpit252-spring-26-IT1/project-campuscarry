...
const PlayerHomePage = () => {
  ...
  const loadPlayerHome = useCallback(async ({ showLoader = !hasLoadedHome } = {}) => {
    ...
    try {
      ...
      const [conversationsResult, recruitersResult, leaderboardEntriesResult] = await Promise.allSettled([
      ...
      let nextConversations = hasLoadedHome ? conversations : [];
      if (conversationsResult.status === "fulfilled") {
        nextConversations = conversationsResult.value;
        setConversations(nextConversations.slice(0, 3));
      } else {
        console.error("Unable to load player home conversations:", conversationsResult.reason);
        if (!hasLoadedHome) {
          setConversations([]);
        }
      }
      ...
      if (recruitersResult.status === "fulfilled") {
        setOpenRecruiters(recruitersResult.value);
      } else {
        console.error("Unable to load recruiter directory for player home:", recruitersResult.reason);
        if (!hasLoadedHome) {
          setOpenRecruiters([]);
        }
      }

      if (leaderboardEntriesResult.status === "fulfilled") {
        setTopPlayers(leaderboardEntriesResult.value.slice(0, 5));
      } else {
        console.error("Unable to load player home leaderboard entries:", leaderboardEntriesResult.reason);
        if (!hasLoadedHome) {
          setTopPlayers([]);
        }
      }
      ...
      try {
        ...
        console.error("Unable to build player home chat notifications:", error);
        if (!hasLoadedHome) {
  ...
  }, [conversations, hasLoadedHome, user.id]);
