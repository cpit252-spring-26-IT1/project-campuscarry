...
import { getApiErrorMessage } from "./auth/shared";
...
const getConversationsForUser = async (userId, options = {}) => {
  ...
  const conversationRequestConfig = includeAllConversations
    ? {
        params: {
          all: true,
        },
  ...
  const conversationSummaries = await safeGetCollection(
    "/chat_threads/conversations",
    conversationRequestConfig,
    { forceRefresh },
  );

  const relevantUserIds = normalizeUniqueUserIds(
    conversationSummaries.flatMap((conversation) => getThreadParticipantIds(conversation)),
  );
  ...
  return conversationSummaries
    .map((conversation) => {
      const conversationKey = getThreadParticipantKey(conversation);
      const participantIds = getThreadParticipantIds(conversation);
...
const getMessagesForThread = async ({ threadId, allowObserverAccess = false }) => {
  const requestConfig = allowObserverAccess
    ? {
        params: {
          all: true,
        },
  ...
  const messages = await safeGetCollection(`/chat_threads/${threadId}/messages`, requestConfig, {
    forceRefresh: true,
  });

  return messages.sort((a, b) => toTimestamp(a.createdAt) - toTimestamp(b.createdAt));
};
...
const sendMessage = async ({ threadId, senderId, body }) => {
  try {
    ...
    throw new Error(getApiErrorMessage(error, "Unable to send message right now."));
  }
};
...
const buildUnreadChatNotificationsFromConversations = async (
  conversations,
  { forceRefresh = false } = {},
) => {
  ...
  const senderIds = normalizeUniqueUserIds(
    unreadConversations.map((conversation) => conversation.lastSenderId),
  );
  ...
  const items = unreadConversations
    .map((conversation) => {
      const sender = usersById.get(normalizeUserId(conversation.lastSenderId));

      return {
        threadId: conversation.id,
        senderId: normalizeUserId(conversation.lastSenderId),
        ...
        participantIds: Array.isArray(conversation?.participantIds)
          ? conversation.participantIds.map((participantId) => normalizeUserId(participantId))
          : [],
        unreadCount: Number(conversation.unreadCount || 0),
      };
    })
  ...
  return {
    unreadCount: unreadConversations.reduce(
      (totalUnreadCount, conversation) =>
        totalUnreadCount + Number(conversation.unreadCount || 0),
      0,
    ),
    items,
  };
...
const getUnreadChatNotifications = async (userId, options = {}) => {
  const normalizedUserId = normalizeUserId(userId);
  const forceRefresh = Boolean(options.forceRefresh);
  const conversations = await getConversationsForUser(normalizedUserId, { forceRefresh });
  return buildUnreadChatNotificationsFromConversations(conversations, { forceRefresh });
};
